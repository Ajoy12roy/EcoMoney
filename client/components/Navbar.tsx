...
export default function Navbar() {
  ...
  // Helper to generate a unique key for the image based on URL
  const imageKey = userData?.image ? `${userData.image}-${new Date().getTime()}` : 'default';
  ...
  return (
    <nav className="w-full bg-white border-b border-slate-100 sticky top-0 z-50">
        ...
        <div className="flex items-center gap-3 md:gap-4">
          <Link href="/profile" className="relative group">
            <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-slate-100 border-2 border-white shadow-sm overflow-hidden transition-all hover:border-emerald-400 hover:shadow-md shrink-0">
              <img 
                // Using crossOrigin fixes the browser warning
                // Using a key forces React to update the image if the URL changes
      ...
      {/* Mobile Menu omitted for brevity but logic remains same */}
      {isMobileMenuOpen && (
