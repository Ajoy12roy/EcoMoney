...
// --- Reusable Editable Component (Futuristic Style) ---
...
const EditableField: React.FC<EditableFieldProps> = ({ label, value, icon, onSave }) => {
  ...
  return (
    <motion.div 
      layout
      className={`group relative flex items-center justify-between p-3 md:p-4 rounded-2xl transition-all duration-300
        ${isEditing ? 'bg-white shadow-[0_4px_20px_rgba(0,0,0,0.05)] border border-indigo-100' : 'hover:bg-white/50 border border-transparent'}
      `}
    >
      <div className="flex-1 mr-3 md:mr-4 flex items-center gap-3 md:gap-4">
        {/* Icon Container */}
        <div className={`p-2.5 md:p-3 rounded-xl transition-colors duration-300 ${isEditing ? 'bg-indigo-600 text-white' : 'bg-indigo-50 text-indigo-500'}`}>
          {icon}
        </div>
          ...
          <div className="relative h-7 md:h-8 flex items-center">
            {isEditing ? (
              <div className="relative w-full">
                ...
                <motion.div 
                  layoutId="underline"
                  className="absolute bottom-0 left-0 h-0.5 w-full bg-linear-to-r from-indigo-500 to-purple-500" 
                />
              </div>
            ) : (
              <motion.p 
                initial={{ opacity: 0 }} 
                animate={{ opacity: 1 }}
                className="text-base md:text-lg font-bold text-slate-700 truncate"
              >
                {value}
              </motion.p>
            )}
      ...
      <motion.button
        ...
        className={`relative flex items-center justify-center gap-2 px-3 py-1.5 md:px-4 md:py-2 text-[10px] md:text-xs font-bold uppercase tracking-wide rounded-xl transition-all shadow-md overflow-hidden shrink-0
          ${isEditing 
            ? 'text-white bg-linear-to-r from-emerald-400 to-emerald-600 shadow-emerald-200' 
            : 'text-indigo-600 bg-white border border-indigo-100 shadow-indigo-100 group-hover:border-indigo-200'
          }`}
      >
        <AnimatePresence mode='wait'>
          {isEditing ? (
            ...
            >
              <Check size={14} strokeWidth={3} /> <span className="hidden sm:inline">Done</span>
            </motion.div>
          ) : (
            <motion.div 
              key="edit"
  ...
  return (
    <div className="min-h-screen bg-[#103b67] flex items-center justify-center p-4 md:p-8 font-sans">
      ...
      <motion.div
        ...
        className="relative w-full max-w-3xl bg-white/70 backdrop-blur-2xl rounded-3xl md:rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-white/50 overflow-hidden"
      >
        {/* Header Decoration */}
        ...
        <div className="relative p-6 md:p-12 flex flex-col md:flex-row gap-8 md:gap-12">
          ...
          <div className="flex flex-col items-center md:w-1/3 border-b border-indigo-50 pb-8 md:border-b-0 md:border-r md:pb-0 md:pr-8">
            ...
            <h2 className="text-xl md:text-2xl font-black text-slate-800 tracking-tight text-center">{name}</h2>
          ...
          <div className="flex-1 space-y-5 md:space-y-6">
            ...
            <div className="space-y-3 md:space-y-4">
              ...
              <EditableField 
                label="Phone Number" 
                value={phone} 
                icon={<Smartphone size={18} className="md:w-5 md:h-5" />}
                onSave={setPhone} 
              />
            ...
            <div className="pt-6 md:pt-8 flex flex-col-reverse sm:flex-row items-center justify-between gap-6 sm:gap-4">
              <button className="text-sm font-semibold text-red-400 hover:text-red-500 transition-colors w-full sm:w-auto text-center">
                Reset Password
              </button>
